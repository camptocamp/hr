...
long as the build is green!) and push it on the registry as `camptocamp/bso_odoo:x.y.z`
...
* Paul tests the image `camptocamp/bso_odoo:9.4.0` and oops, it seems he
...
2. Create local patch branch
    ...
    * Create PR to patch remote project branch.
        ```git
        camptocamp/bso/patch-x.y.next_z <- <user_name>/bso/<your_patch_branch>
        ```
