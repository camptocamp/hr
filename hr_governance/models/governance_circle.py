...
from odoo.tools import html_to_inner_content, ormcache
...
class GovernanceCircle(models.Model):
    ...
    member_cnt = fields.Integer(compute="_compute_member_cnt")
    ...
    def _compute_is_editable(self):
        for rec in self:
            rec.is_editable = bool(not rec.type_id or rec.type_id.type == "template")
    ...
    def _compute_is_addable(self):
        for rec in self:
            ...
            if is_single_mode_on:
                rec.is_addable = not rec.is_circle and not len(rec.member_ids)
            else:
                rec.is_addable = True

    @api.depends("member_ids")
    def _compute_member_cnt(self):
        for rec in self:
            rec.member_cnt = len(rec.member_ids)
    ...
    def create(self, vals):
        for val in vals:
            ...
            if context.get("default_is_circle", False):
                circle = self.new(val)
                new_roles_vals = circle._get_structuring_role_vals()
    ...
    def write(self, vals):
        ...
        allowed_to_edit = (
            self.id in self.env.user.allowed_edit_governance_ids.ids
            or self.env.user.user_has_groups("hr_governance.governance_group_manager")
        )
        if not allowed_to_edit:
            if self.is_circle:
                editable_roles = [
                    role["name"] for role in self._get_enable_edit_circle_role()
                ]
    ...
    @api.model
    def _get_steering_role_user_ids(self, circle_id):
        ...
        steering = self
        if circle_id.is_circle:
            steering |= circle_id.child_ids.filtered_domain(domain)
        else:
            # if it's role, retrieve steering roles of encompassing circle
            steering |= circle_id._get_hierarchy_records(
                include_self=True
