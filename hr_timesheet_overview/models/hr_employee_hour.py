...
class HrEmployeeHour(models.Model):
    ...
    _sql_constraints = [
        (
            ...
            _("You can't have two hour lines with same fields: {})").format(
    ...
    def action_generate_data(self, employee_ids=None):
        ...
        for employee in employees:
            ...
            att_vals = self._get_attendances_values(employee, date_from)
            self.create(att_vals)
            ts_vals = self._prepare_timesheets_lines(employee, date_from)
            self.create(ts_vals)
