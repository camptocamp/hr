...
class HrCourse(models.Model):
    ...
    schedule_validity_end_date = fields.Date(
        related="course_schedule_ids.validity_end_date"
    )
    ...
    def send_course_notification_email(self):
        ...
        if channel and self._name == "hr.course":
            channel.with_context(
                ...
                attendees=self.course_attendee_ids.mapped("employee_id.name"),
    ...
    def process_validity(self):
        ...
        for course in self:
            if course.schedule_validity_end_date:

                if (
                    course.schedule_validity_end_date
