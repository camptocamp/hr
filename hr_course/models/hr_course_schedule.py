...
class HrCourseSchedule(models.Model):
    ...
    def send_course_notification_email(self):
        ...
        email_values = email_template.generate_email(self.ids, ["body_html"])

        values = email_values[self.id]
        self.with_context(
            recipient_ids=channel.channel_last_seen_partner_ids.mapped("partner_email"),
            email_from=channel.alias_id.display_name,
        ).message_post(
            body=values["body_html"], channel_ids=channel.ids, message_type="email"
        )
        return True
        ...
        for course_schedule in self:
            if course_schedule.validity_end_date:
                if (
                    ...
                    course_schedule.alerted = True
                    course_schedule.send_course_notification_email()
