...
OrgChart.prototype.load = function (t) {
    ...
    this.server.read(t, this.width(), this.height(), e, m, O, function (t) {
        m != OrgChart.action.exporting && (y.nodes = t.nodes, y.visibleNodeIds = t.visibleNodeIds, y.roots = t.roots), y.editUI.fields = t.allFields;
        var e = {
            defs: ""
        };
        ...
        OrgChart.events.publish("prerender", [y, e]), r = e.content;
        for (var o = 0; o < t.visibleNodeIds.length; o++) {
            var l = t.nodes[t.visibleNodeIds[o]],
                s = y._get(l.id);
            r += y.ui.node(l, s, t.animations, y.config, void 0, void 0, void 0, m, i, y)
        }
        for (o = 0; o < t.visibleNodeIds.length; o++) {
            l = t.nodes[t.visibleNodeIds[o]], s = y._get(l.id);
            r += y.ui.link(l, y, i, t.bordersByRootIdAndLevel, t.nodes), r += y.ui.expandCollapseBtn(y, l, y._layoutConfigs, m, i)
        }
        e = {
            content: r,
            res: t
        };
        if (OrgChart.events.publish("render", [y, e]), r = e.content, r += y.ui.lonely(y.config), m !== OrgChart.action.exporting) {
            m !== OrgChart.action.centerNode && m !== OrgChart.action.insert && m !== OrgChart.action.expand && m !== OrgChart.action.collapse && m !== OrgChart.action.update || (n = a), m === OrgChart.action.init && null != a && (n = a), y.response = t;
            C = y.ui.svg(y.width(), y.height(), n, y.config, r);
            if (y._initialized) {
                var h = y.getSvg(),
                    d = h.parentNode;
                d.removeChild(h), d.insertAdjacentHTML("afterbegin", C), y._attachEventHandlers(), y.xScrollUI.addListener(y.getSvg()), y.yScrollUI.addListener(y.getSvg()), y.xScrollUI.setPosition(), y.yScrollUI.setPosition()
            } else y.element.innerHTML = y.ui.css() + y.ui.exportMenuButton(y.config) + C, y._attachInitEventHandlers(), y._attachEventHandlers(), y.xScrollUI.create(y.width(), y.config.padding), y.xScrollUI.setPosition(), y.xScrollUI.addListener(y.getSvg()), y.yScrollUI.create(y.height(), y.config.padding), y.yScrollUI.setPosition(), y.yScrollUI.addListener(y.getSvg()), y.config.enableSearch && y.searchUI.addSearchControl(), y.toolbarUI.init(y, y.config.toolbar);
            var c = !1;
            y.notifierUI.show(t.notif);
            var g = y.response.animations;
            if (0 < g[0].length) {
                y._hideBeforeAnimation(g[0].length);
                for (o = 0; o < g[0].length; o++) g[0][o] = y.getNodeElement(g[0][o]);
            ...
            m === OrgChart.action.centerNode ? OrgChart.anim(y.getSvg(), {
                ...
                y.ripple(O.options.rippleId), c || (x && x(), OrgChart.events.publish("redraw", [y]), y._showAfterAnimation(), c = !0)
            }, function () {
                y.xScrollUI.setPosition(), y.yScrollUI.setPosition()
            }) : !a || !y.response || a[0] == y.response.viewBox[0] && a[1] == y.response.viewBox[1] && a[2] == y.response.viewBox[2] && a[3] == y.response.viewBox[3] || m !== OrgChart.action.insert && m !== OrgChart.action.expand && m !== OrgChart.action.collapse && m !== OrgChart.action.update && m !== OrgChart.action.init ? 0 == g[0].length && (c || (x && x(), OrgChart.events.publish("redraw", [y]), c = !0)) : OrgChart.anim(y.getSvg(), {
                viewbox: a
            }, {
                viewbox: y.response.viewBox
            ...
            var p = t.boundary,
                f = p.maxX - p.minX,
                u = p.maxY - p.minY,
                C = y.ui.svg(f, u, [p.minX, p.minY, f, u], y.config, r, i);
            x(C)
        }
    }, function (t) {
    ...
    if (null != i) {
        ...
        if (null != a) {
            ...
            var h = i.w,
                ...
                O = OrgChart.randomId();
            u.setAttribute("id", O);
            var x = OrgChart.t(i.templateName, i.min, this.getScale());
            C.setAttribute("x", x.ripple.rect ? x.ripple.rect.x : 0), C.setAttribute("y", x.ripple.rect ? x.ripple.rect.y : 0), C.setAttribute("width", x.ripple.rect ? x.ripple.rect.width : i.w), C.setAttribute("height", x.ripple.rect ? x.ripple.rect.height : i.h), C.setAttribute("rx", x.ripple.radius), C.setAttribute("ry", x.ripple.radius), m.setAttribute("clip-path", "url(#" + O + ")"), m.setAttribute("cx", o), m.setAttribute("cy", l), m.setAttribute("r", 0), m.setAttribute("fill", x.ripple.color), m.setAttribute("class", "ripple"), u.appendChild(C), f.appendChild(u), f.appendChild(m), a.appendChild(f), OrgChart.anim(m, {
                r: 0,
                opacity: 1
            }, {
                r: p,
    ...
    var e = this.get(t);
    if (e)
        for (var r = this.config.nodes.length - 1; 0 <= r; r--) this.config.nodes[r].pid != t && this.config.nodes[r].stpid != t || (this.config.nodes[r].pid = e.pid, this.config.nodes[r].stpid = e.stpid), this.config.nodes[r].id == t && this.config.nodes.splice(r, 1);
    return this
    ...
    var e = this.get(t),
        ...
        i = {};
    if (e)
        for (var a = this.config.nodes.length - 1; 0 <= a; a--) this.config.nodes[a].pid == t ? r[this.config.nodes[a].id] = e.pid : this.config.nodes[a].stpid == t && (i[this.config.nodes[a].id] = e.stpid);
    return {
        newPidsForIds: r,
        newStpidsForIds: i
    }
    ...
    Array.isArray(c) || (c = [c]), Array.isArray(g) || (g = [g]), Array.isArray(p) || (p = [p]), O = setInterval(function () {
        ...
        m && m(), 1 + y < (x += 1) && (clearInterval(O), C && C(c))
    }, 10)
...
OrgChart.editUI.prototype._addField = function (t, e, r, i) {
    ...
    else if ((0 < navigator.userAgent.toLowerCase().indexOf("safari") || 0 < navigator.userAgent.toLowerCase().indexOf("firefox")) && window.location !== window.parent.location) e && e(!1);
    else {
    ...
    o.margin && o.margin[0] < 2 && (o.margin[0] = 2), o.margin && o.margin[1] < 2 && (o.margin[1] = 2), o.margin && o.margin[2] < 2 && (o.margin[2] = 2), o.margin && o.margin[3] < 2 && (o.margin[3] = 2), this._draw(!1, OrgChart.action.exporting, t, function (t) {
        ...
        if ("svg" == o.ext)
            if (l) l(o, r.innerHTML);
                ...
                if (OrgChart._downloadFile(o.mime, a.content, a.options.filename, a.options.openInNewTab), !1 === (n = OrgChart.events.publish("exportend", [s, a]))) return !1
            }
            ...
            // l || OrgChart.loading.show(s), l ? l(s, e, r.querySelector("svg")) : (e = JSON.stringify(e),
            // OrgChart._ajax(s.config.exportUrl + "/v3", "POST", e, "arraybuffer", function (t) {
            // if (!1 === OrgChart.events.publish("exportend", [s, t])) return !1;
            // OrgChart.loading.hide(s), OrgChart._downloadFile(o.mime, t, o.filename, o.openInNewTab)
            // }))
        })
    })
    ...
    for (var g = d.w * (100 / r), p = d.h * (100 / r), f = o, u = l, C = []; f < s + o;) {
        for (; u < h + l;) {
            var m = [f, u, g, p];
            m = m.join(), C.push({
                vb: m,
...
}, OrgChart.prototype._nodeMouseDownHandler = function (O, t, i) {
    if (this.config.enableDragDrop) {
        var e = OrgChart._getClientXY(t),
            r = O.getAttribute("node-id"),
            x = this.getNode(r);
        O._dragEventFired = !1;
        var y = null,
            v = null;
        ...
        var b = this,
            ...
            k = OrgChart._getTransform(O),
            S = k[4],
            I = k[5],
            A = b.getScale(),
            L = O.cloneNode(!0);
        w.insertBefore(L, w.firstChild), L.style.opacity = .7;
        var M = function (t, e) {
                if (null != t) {
                    e.style.opacity = 1;
                    for (var r = b.getNode(y), i = OrgChart.getStParentNodes(r), a = 0; a < i.length; a++) {
                ...
                if (_) {
                    ...
                    if (a.left || a.right || a.up || a.down) {
                        ...
                        b.startMove(a, function (t) {
                            k[4] = n + t.x, k[5] = o + t.y, _.x = l - t.xWithoutScale, _.y = s - t.yWithoutScale, e.x = h - t.xWithoutScale, e.y = d - t.yWithoutScale, L.setAttribute("transform", "matrix(" + k.toString() + ")")
                        })
                    } else {
                        if (b.stopMove(), w.classList && (w.classList.add("cursor-grab"), w.classList.remove("cursor-move"), w.classList.remove("cursor-nodrop"), w.classList.remove("cursor-copy")), M(y, v), v = y = null, b.config.enableDragDrop)
                            for (; null != r && r != w;) {
                        ...
                        if (null != y) {
                            v.style.opacity = .5;
                            for (var g = b.getNode(y), p = OrgChart.getStParentNodes(g), f = 0; f < p.length; f++) {
                                var u = b.getNodeElement(p[f].id);
                                u && (u.style.opacity = .1)
                            }
                            !b._canUpdateLink(x.id, y) && w.classList ? (w.classList.remove("cursor-grab"), w.classList.remove("cursor-move"), w.classList.remove("cursor-copy"), w.classList.add("cursor-nodrop")) : w.classList && (w.classList.remove("cursor-grab"), w.classList.remove("cursor-move"), w.classList.add("cursor-copy"), w.classList.remove("cursor-nodrop"))
                        }
                        var C = (e.x - _.x) / A,
                            m = (e.y - _.y) / A;
                        if (k[4] = S + C, k[5] = I + m, !O._dragEventFired && (Math.abs(e.x - _.x) > OrgChart.FIRE_DRAG_NOT_CLICK_IF_MOVE || Math.abs(e.y - _.y) > OrgChart.FIRE_DRAG_NOT_CLICK_IF_MOVE)) !1 === OrgChart.events.publish("drag", [b, c]) && E(), O._dragEventFired = !0;
                        L.setAttribute("transform", "matrix(" + k.toString() + ")")
                    }
                }
            },
            E = function (t) {
                if (b.stopMove(), w.classList && (w.classList.remove("cursor-grab"), w.classList.remove("cursor-move"), w.classList.remove("cursor-nodrop"), w.classList.remove("cursor-copy")), w.removeEventListener(i.move, a), w.removeEventListener(i.up, E), i.leave && w.removeEventListener(i.leave, E), x.id == y || null == y) return w.removeChild(L), void (b._gragStartedId = null);
                var e = b.getNode(y);
                if (!1 === OrgChart.events.publish("drop", [b, x.id, e.id])) return M(y, v), w.removeChild(L), void (b._gragStartedId = null);
                if (b._canUpdateLink(x.id, y)) {
                    var r = b.get(x.id);
                    r.pid = y, r.stpid = null, b.updateNode(r, null, !0)
                } else w.removeChild(L), M(y, v);
                b._gragStartedId = null
            };
        w.addEventListener(i.move, a), w.addEventListener(i.up, E), i.leave && w.addEventListener(i.leave, E)
    }
...
OrgChart.prototype._mouseDownHandler = function (t, e, r) {
...
}, void 0 === OrgChart && (OrgChart = {}), OrgChart.local = {}, OrgChart.local._setPositions = function (t, e, r) {
    for (var i = {}, a = 0; a < t.length; a++) {
        if ((s = t[a]).stContainerNodes)
            for (var n = s.stContainerNodes.length - 1; 0 <= n; n--) {
    ...
    for (a = 0; a < t.length; a++) {
        ...
        if ((s = t[a]).stContainerNodes)
            for (n = 0; n < s.stContainerNodes.length; n++)
                for (var h = (o = s.stContainerNodes[n]).x, d = o.y, c = 0; c < o.stChildren.length; c++) {
                    var g = o.stChildren[c];
                    OrgChart.local._lastWalk(g, h, d, i[o.id], o.padding)
                }
    }
    r()
    ...
    for (o = 0; o < t.length; o++) {
        ...
        var s, h = 0;
        switch (0 < s && (h = a[s - 1] + l.siblingSeparation), OrgChart.local._secondWalk(t[o], 0, 0, h, i, r, l), a[s] = r.maxY - r.minY, l.orientation) {
            case OrgChart.orientation.right:
            case OrgChart.orientation.right_top:
            case OrgChart.orientation.left:
            case OrgChart.orientation.left_top:
    ...
    if (!t.isPartner) {
        var n = null;
        if (t.x = 0, t.y = 0, t._prelim = 0, t._modifier = 0, t.leftNeighbor = null, t.rightNeighbor = null, OrgChart.local._setLevelMaxNodeHeight(t, e, r, a), OrgChart.local._setNeighbors(t, e, i), OrgChart.local._hasChildren(t) && e != OrgChart.MAX_DEPTH) {
            ...
            var s = OrgChart.local._fChild(t),
                ...
                d = s._prelim + (h._prelim - s._prelim) / 2 + OrgChart.local._leftMidPoint(h, a);
            if (s.isAssistant && h.isAssistant && (d = s._prelim + (h._prelim - s._prelim) / 2 + OrgChart.local._leftMidPoint(s, a)), d -= OrgChart.local._mid(t, a), null != (n = OrgChart.local._getLeftSibling(t))) {
                c = a.siblingSeparation;
                if (t.isAssistant && t.isSplit) c = a.assistantSeparation;
                else if (2 == n.layout) c = a.mixedHierarchyNodesSeparation;
                    ...
                    (c = 15 * n.parent.partnerSeparation + g / 2 - p) < a.siblingSeparation && (c = a.siblingSeparation)
                }
                t._prelim = n._prelim + OrgChart.local._nodeWithPartnersSize(n, a) + c, t._modifier = t._prelim - d, OrgChart.local._apportion(t, e, a)
            } else a.orientation <= 3 ? t._prelim = d : t._prelim = 0
        } else if (null != (n = OrgChart.local._getLeftSibling(t))) {
            var c = a.siblingSeparation;
            if (t.isAssistant) c = a.assistantSeparation;
            else if (2 == n.layout) c = a.mixedHierarchyNodesSeparation;
            else if (n.parent && 4 == n.parent.hasPartners && a.orientation <= 3) {
                var g = OrgChart.local._size(n.parent, a),
                    p = OrgChart.local._rightMidPoint(n, a);
                (c = n.parent.partnerSeparation + g / 2 - p) < a.siblingSeparation && (c = a.siblingSeparation)
            }
            t._prelim = n._prelim + OrgChart.local._nodeWithPartnersSize(n, a) + c
        } else t._prelim = 0
    }
}, OrgChart.local._secondWalk = function (t, e, r, i, a, n, o) {
    if (e <= OrgChart.MAX_DEPTH && !t.isPartner) {
        var l, s, h = t._prelim + r,
            d = i;
        switch (l = a[e], s = OrgChart.local._getNodeHeight(t, o), t.x = h, t.y = d, t.hasPartners && OrgChart.local.nodeHasPartnersPosition(t, o, l), (t.isSplit || t.isAssistant || 2 == t.layout) && (t.y = d + l / 2 - s / 2), o.orientation) {
            case OrgChart.orientation.right:
            case OrgChart.orientation.right_top:
            case OrgChart.orientation.left:
            case OrgChart.orientation.left_top:
        ...
        if (OrgChart._setMinMaxXY(t, n), 0 != t.children) {
            ...
            t.layout != OrgChart.mixed && t.layout != OrgChart.tree || (g = o.mixedHierarchyNodesSeparation);
            var p = 0;
            if (OrgChart.local._secondWalk(t.children[p], e + 1, r + t._modifier, i + l + g, a, n, o), t.children[p].isPartner) {
                for (; t.children.length - 1 > p && t.children[p].isPartner;) p++;
                OrgChart.local._secondWalk(t.children[p], e + 1, r + t._modifier, i + l + g, a, n, o)
            }
        }
    ...
    if (!t.isPartner)
        for (var i = OrgChart.local._fChild(t), a = i.leftNeighbor, n = 1, o = OrgChart.MAX_DEPTH - e; null != i && null != a && n <= o;) {
            ...
            var p = a._prelim + s + OrgChart.local._nodeWithPartnersSize(a, r) + g - (i._prelim + l);
            if (0 < p) {
            ...
            n++, null != (i = OrgChart.local._hasChildren(i) ? OrgChart.local._fChild(i) : OrgChart.local._getLeftmost(t, 0, n)) && (a = i.leftNeighbor)
        }
    ...
    switch (e.orientation) {
        ...
        case OrgChart.orientation.bottom_left:
            ...
            for (var f = 0; f < t.parent.children.length; f++) {
                1 == (u = t.parent.children[f]).isPartner ? (o = t.parent.x + t.parent.w + t.parent.partnerSeparation, g ? (l = t.parent.y - (r[i - 1] - t.parent.h) / 2 + (r[i - 1] - n) / 2, c = u.h, g = !1) : (l += c + e.partnerNodeSeparation, c = u.h), u.x = o, u.y = l) : 2 == u.isPartner && (s = t.parent.x - u.w - t.parent.partnerSeparation, p ? (h = t.parent.y - (r[i - 1] - t.parent.h) / 2 + (r[i - 1] - a) / 2, d = u.h, p = !1) : (h += d + e.partnerNodeSeparation, d = u.h), u.x = s, u.y = h)
            }
            break;
        case OrgChart.orientation.right:
        case OrgChart.orientation.right_top:
    ...
    for (var e = 0; e < t.children.length; e++)
        if (!t.children[e].isPartner) return !0;
    return !1
...
}, OrgChart.server.prototype.read = function (h, d, c, g, p, f, u, C) {
    var m = this;
    OrgChart.state._get(this.config.state, d, c, function (t) {
        m.state = t, m.action = p, m.actionParams = f, p == OrgChart.action.init && m.state ? (m.viewBox = m.state.vb, m.vbIsInitializedFromState = !0) : (m.viewBox = g, m.vbIsInitializedFromState = !1);
        var r = m.maxX,
            i = m.maxY,
            a = m.minX,
            n = m.minY,
            o = m.bordersByRootIdAndLevel,
            l = m.roots,
            s = m.nodes;
        if (h) {
            var e = OrgChart.server._getResponse(d, c, m.visibleNodeIds, m.config, r, i, a, n, m.viewBox, l, m.action, m.actionParams, s, m.oldNodes, m.vbIsInitializedFromState);
            p != OrgChart.action.exporting && (m.maxX = r, m.maxY = i, m.minX = a, m.minY = n, m.roots = l, m.nodes = s, m.visibleNodeIds = e.visibleNodeIds), e.bordersByRootIdAndLevel = o, e.roots = l, e.adjustify = {
                x: 0,
                y: 0
            }, m.state && (e.adjustify = m.state.adjustify), u(e)
        } else m.oldNodes = s || null, m._read(function (t) {
            r = t.maxX, i = t.maxY, a = t.minX, n = t.minY, o = t.bordersByRootIdAndLevel, l = t.roots, s = t.nodes;
            var e = OrgChart.server._getResponse(d, c, m.visibleNodeIds, m.config, r, i, a, n, m.viewBox, l, m.action, m.actionParams, s, m.oldNodes, m.vbIsInitializedFromState);
            e.notif = t.limit, e.roots = l, e.bordersByRootIdAndLevel = o, e.adjustify = t.adjustify, p != OrgChart.action.exporting && (m.maxX = r, m.maxY = i, m.minX = a, m.minY = n, m.roots = l, m.nodes = s, m.visibleNodeIds = e.visibleNodeIds, m.bordersByRootIdAndLevel = o), u(e)
        }, C)
    })
    ...
    for (s = 0; s < o.length && s != OrgChart.SEARCH_RESULT_LIMIT; s++) {
        ...
        if (!g) {
            ...
            var x = OrgChart._getTemplate(o[s].tags, this.config.tags, this.config.template);
            l.push({
                id: o[s].id,
                node: new OrgChart.node(o[s], null, [], x),
                textId: u,
                textInNode: f,
                text: C,
                img: m
    ...
    if (t.min || a != OrgChart.action.minimize || s.id != t.id || (t.min = !0), !0 === t.min && a == OrgChart.action.maximize && s.id == t.id && (t.min = !1), a == OrgChart.action.init && null != d && (t.min = d.min.has(t.id)), !t.min)
    ...
    0 < u && (t.subLevels = u), -1 != t.tags.indexOf("assistant") && (t.isAssistant = !0)
    ...
    if (OrgChart.server._initNode(e, r, s, i, c, h, d, g, p, f, u), e.isAssistant && (n[e.pid] || (n[e.pid] = []), n[e.pid].push(e.id)), 0 < e.subLevels && o.push(e.id), !e.isAssistant && 0 == e.childrenIds.length && e.parent && !l[e.pid]) {
        for (var C = [], m = 0, O = 0, x = 0; x < e.parent.children.length; x++) {
            var y = e.parent.children[x];
            -1 == y.tags.indexOf("partner") && -1 == y.tags.indexOf("assistant") && 0 == y.childrenIds.length ? C.push(y.id) : -1 != y.tags.indexOf("assistant") ? m++ : -1 != y.tags.indexOf("partner") && O++
        }
        OrgChart.MIXED_LAYOUT_ALL_NODES && 1 < C.length && C.length == e.parent.childrenIds.length - m - O ? l[e.pid] = C : !OrgChart.MIXED_LAYOUT_ALL_NODES && 1 < C.length && (l[e.pid] = C)
    }
    ...
    for (var o = a.nodes, l = {}, s = [], h = [], d = 0; d < o.length; d++) {
        var c = o[d],
            g = Array.isArray(c.tags) ? c.tags.slice(0) : [],
            p = OrgChart._getTemplate(g, a.tags, a.template),
            f = new OrgChart.node(c.id, c.pid, g, p);
        null != c.ppid && (f.ppid = c.ppid), c.stpid && (f.stpid = c.stpid), null != a.orderBy && (f.order = OrgChart.server._getOrderFieldValue(c, a.orderBy)), l[c.id] = f, s.push(c.id)
    }
    ...
    for (d = 0; d < s.length; d++) {
        var u = s[d],
            ...
            m = l[f.stpid],
            O = l[f.pid];
        if (m || (f.stpid = null), O || (f.pid = null), m) {
            var x = i ? i[m.id] : null;
            x && (m.min = x.min), m.stChildrenIds.push(f.id)
        } else O ? (C && (f.collapsed = C.collapsed, f.min = C.min), O.childrenIds.push(f.id)) : (C && (f.collapsed = C.collapsed, f.min = C.min), h.push(f));
    ...
    if (e == OrgChart.action.init && a.expand && a.expand.nodes && null == n)
        for (d = 0; d < a.expand.nodes.length; d++)
            for (f = l[a.expand.nodes[d]]; f;) v.push(f.id), null == f.pid && null != f.stpid ? (f = l[f.stpid]).min = !1 : f = l[f.pid];
    else if (e == OrgChart.action.centerNode) {
        for (var b = l[r.id]; b;) {
            if (v.push(b.id), r.options.parentState === OrgChart.COLLAPSE_PARENT_NEIGHBORS && b)
                for (d = 0; d < b.childrenIds.length; d++) {
                    (U = l[b.childrenIds[d]]).collapsed = !0
                }
        ...
        for (f = l[r.insertedNodeId]; f;) v.push(f.id), null == f.pid && null != f.stpid ? (f = l[f.stpid]).min = !1 : f = l[f.pid];
    var k = [],
        S = {},
        I = [],
        A = {},
        L = [];
    for (d = 0; d < h.length; d++) OrgChart.server._iterate(h[d], h[d], l, 0, k, S, I, A, "", t, a, e, v, r, L, n);
    ...
    for (d = 0; d < L.length; d++) {
        ...
        for (w = 0; w < f.children.length; w++) {
            (U = f.children[w]).isAssistant || (-1 != U.tags.indexOf("right-partner") ? (U.isPartner = 1, U.children = [], E.push(U.id)) : -1 != U.tags.indexOf("left-partner") ? (U.isPartner = 2, U.children = [], N.push(U.id)) : -1 == U.tags.indexOf("partner") || T % 2 ? -1 != U.tags.indexOf("partner") && T % 2 ? (U.isPartner = 2, U.children = [], N.push(U.id), T++) : null != U.ppid ? (P[U.ppid] || (P[U.ppid] = []), P[U.ppid].push(U.id)) : M.push(U.id) : (U.isPartner = 1, U.children = [], E.push(U.id), T++))
        }
        f.children = [];
        ...
        for (w = N.length - 1; 0 <= w; w--)
            if (P[N[w]])
                for (var j = 0; j < P[N[w]].length; j++) f.children.push(l[P[N[w]][j]]), B++, -1 == z.indexOf(N[w]) && z.push(N[w]);
        ...
        for (w = 0; w < E.length; w++)
            if (P[E[w]])
                for (j = 0; j < P[E[w]].length; j++) f.children.push(l[P[E[w]][j]]), F++, -1 == H.indexOf(E[w]) && H.push(E[w]);
        f.partnerSeparation = Math.max(z.length, H.length) * a.partnerChildrenSplitSeparation + a.minPartnerSeparation, M.length || !B || F ? M.length || B || !F ? M.length || 1 != B || 1 != F ? M.length || B || F ? !M.length || B || F ? M.length && (B || F) ? f.hasPartners = 7 : f.hasPartners = 1 : f.hasPartners = 6 : f.hasPartners = 5 : f.hasPartners = 4 : f.hasPartners = 3 : f.hasPartners = 2
    }
    for (d = 0; d < I.length; d++)
        for (f = l[I[d]], w = 0; w < f.subLevels; w++) {
            var R = new OrgChart.node(f.id + "_sub_level_index_" + w, f.pid, [], "subLevel");
            if (OrgChart.server._initDinamicNode(R, f.lcn), O = f.parent) {
                var Y = O.children.indexOf(f);
                -1 < Y && (O.children.splice(Y, 1), O.children.splice(Y, 0, R)), R.children.push(f), R.parent = O, f.parent = R, l[R.id] = R
            }
        }
    for (var X in S) {
        (O = l[X]).hasAssistants = !0;
        R = new OrgChart.node(O.id + "_split_0", O.id, ["assistant"], "split");
        OrgChart.server._initDinamicNode(R, O.lcn, !0), l[R.id] = R;
        var D = [];
        for (w = O.children.length - 1; 0 <= w; w--) {
            (U = O.children[w]).isAssistant ? (U.parent = null, O.children.splice(w, 1), D.splice(0, 0, U.id)) : U.isPartner || ((U.parent = R).children.unshift(U), O.children.splice(w, 1))
        }
        if (D.length % 2) {
            var q = l[D[D.length - 1]],
                V = new OrgChart.node(q.id + "_mirror", q.pid, [], "mirror");
            OrgChart.server._initDinamicNode(V, q.lcn, !0), q._m = V.id, V.isAssistant = !0, V.w = q.w, V.h = q.h, l[V.id] = V, D.splice(D.length - 1, 0, V.id)
        }
        var W = 1;
        for (w = D.length - 1; 0 <= w; w--)
            if (w % 2 && w != D.length - 1) {
                var G = new OrgChart.node(O.id + "_split_" + W, null, [], "split");
                OrgChart.server._initDinamicNode(G, O.lcn, !0), l[G.id] = G, D.splice(w, 0, G.id), W++
            } else w % 2 && D.splice(w, 0, R.id);
    ...
    var $ = !1;
    for (var tt in t) {
        if ((at = t[tt]).layout == OrgChart.mixed) {
            $ = !0;
            break
        }
    }
    if ($)
        for (var X in A) {
            if ((at = t[(O = l[X]).lcn ? O.lcn : "base"]).layout == OrgChart.mixed) {
                var et = A[X];
                for (d = et.length - 1; 0 <= d; d--) {
                    if (O = (U = l[et[d]]).parent, U.layout = OrgChart.mixed, 0 < d) {
                        for (w = O.children.length - 1; 0 <= w; w--) U.id == O.children[w].id && O.children.splice(w, 1);
                        var rt = l[et[d - 1]];
                        U.parent = rt, U.layout = OrgChart.mixed, rt.children.push(U)
                    }
                }
            }
        }
    var it = !1;
    for (var tt in t) {
        if ((at = t[tt]).layout == OrgChart.tree || at.layout == OrgChart.treeRightOffset || at.layout == OrgChart.treeLeftOffset) {
            it = !0;
            break
        }
    }
    if (it)
        for (var X in A) {
            var at;
            if ((at = t[(O = l[A[X][0]].parent).lcn ? O.lcn : "base"]).layout == OrgChart.tree || at.layout == OrgChart.treeRightOffset || at.layout == OrgChart.treeLeftOffset) {
                R = new OrgChart.node(O.id + "_split_0", O.id, [], "split");
                OrgChart.server._initDinamicNode(R, O.lcn), (l[R.id] = R).layout = OrgChart.tree;
                var nt = [];
                for (d = A[X].length - 1; 0 <= d; d--) {
                    for (U = l[A[X][d]], w = 0; w < O.children.length; w++) O.children[w].id == U.id && O.children.splice(w, 1);
                    if (U.parent = null, U.layout = OrgChart.tree, at.layout == OrgChart.treeRightOffset && nt.splice(0, 0, U.id), at.layout == OrgChart.treeLeftOffset || at.layout == OrgChart.treeRightOffset) {
                        var ot = new OrgChart.node(U.id + "_mirror", null, [], "mirror");
                        OrgChart.server._initDinamicNode(ot, U.lcn), ot.layout = OrgChart.tree, l[ot.id] = ot, nt.splice(0, 0, ot.id)
                    }
                    at.layout != OrgChart.treeRightOffset && nt.splice(0, 0, U.id)
                }
                for (W = 1, w = nt.length - 1; 0 <= w; w--)
                    if (w % 2 && w != nt.length - 1) {
                        G = new OrgChart.node(O.id + "_split_" + W, null, [], "split");
                        OrgChart.server._initDinamicNode(G, O.lcn), G.layout = OrgChart.tree, l[G.id] = G, nt.splice(w, 0, G.id), W++
                    } else w % 2 && nt.splice(w, 0, R.id);
    ...
    var u = h[0],
        ...
        x = a - o + 2 * i.padding,
        y = n - l + 2 * i.padding,
        v = OrgChart.getScale(s, t, e, i.scaleInitial, i.scaleMax, i.scaleMin, x, y);
    if (m.top = l - i.padding, m.left = o - i.padding, m.bottom = n + i.padding - e / v, m.right = a + i.padding - t / v, m.maxX = a, m.minX = o, m.maxY = n, m.minY = l, null != s || f || i.align != OrgChart.CENTER) {
        if (null == s && !f && i.align == OrgChart.ORIENTATION) {
    ...
    if (d == OrgChart.action.centerNode || d == OrgChart.action.maximize) {
        var S = g[c.id];
        1 == c.options.horizontal && (s[0] = S.x + S.w / 2 - s[2] / 2), 1 == c.options.vertical && (s[1] = S.y + S.h / 2 - s[3] / 2), i.sticky && (s[0] < m.left && s[0] < m.right && (s[0] = m.left > m.right ? m.right : m.left), s[0] > m.right && s[0] > m.left && (s[0] = m.left > m.right ? m.left : m.right), s[1] < m.top && s[1] < m.bottom && (s[1] = m.top > m.bottom ? m.bottom : m.top), s[1] > m.bottom && s[1] > m.top && (s[1] = m.top > m.bottom ? m.top : m.bottom))
    }
    ...
    for (var L = 0; L < h.length; L++) OrgChart.server._iterate2(h[L], g, i, s, d, c, C, p, r, O);
    return {
        animations: O,
        boundary: m,
        viewBox: s,
    ...
    for (var C = 0; C < t.stChildren.length; C++) OrgChart.server._iterate2(t.stChildren[C], e, r, i, a, n, o, l, s, h);
    for (C = 0; C < t.children.length; C++) OrgChart.server._iterate2(t.children[C], e, r, i, a, n, o, l, s, h)
    ...
    node: function (t, e, r, i, a, n, o, l, s, h) {
        ...
        var m = "";
        t.lcn && (m = 'lcn="' + t.lcn + '"');
        var O = OrgChart.nodeOpenTag.replace("{lcn}", m).replace("{id}", t.id).replace("{class}", C).replace("{sl}", t.sl).replace("{x}", u.x).replace("{y}", u.y),
            x = OrgChart._getOpacity(r, t);
        (O = O.replace("{opacity}", x), null != i.nodeMenu && "split" != t.templateName && l !== OrgChart.action.exporting) && (c += d.nodeMenuButton.replace("{id}", t.id).replace("{cw}", t.w / 2).replace("{ew}", t.w - (t.padding ? t.padding[1] : 0)));
        return c = O + c + OrgChart.grCloseTag
    },
    ...
    link: function (t, e, r, i, a) {
        ...
        if (t.hasPartners) {
            ...
            for (var f = 0; f < t.children.length; f++) {
                var u = null;
                null != (C = t.children[f]).ppid && (u = a[C.ppid]), u ? (p.ppnodes[C.id] = u, p.ids.push(C.id), 1 == u.isPartner ? (-1 == p.rightIds.indexOf(u.id) && p.rightIds.push(u.id), p.indexes[C.id] = p.rightIds.indexOf(u.id), p.firstRight || (p.firstRight = C)) : 2 == u.isPartner && (-1 == p.leftIds.indexOf(u.id) && p.leftIds.push(u.id), p.indexes[C.id] = p.leftIds.indexOf(u.id), p.lastLeft = C)) : C.isPartner || (p.lastLeft = C, p.firstRight || (p.firstRight = C))
            }
        ...
        for (f = 0; f < t.children.length; f++) {
            var C = t.children[f],
                ...
                x = (l = OrgChart.t(C.templateName, C.min, r)).link;
            if (p && -1 != p.ids.indexOf(C.id)) switch (o.orientation) {
                case OrgChart.orientation.top:
                case OrgChart.orientation.top_left:
                    O = OrgChart.ui._linkPpTop(p, t, C, m, g, l);
                    break;
                case OrgChart.orientation.bottom:
                case OrgChart.orientation.bottom_left:
                    O = OrgChart.ui._linkPpBottom(p, t, C, m, g, l);
                    break;
                case OrgChart.orientation.right:
                case OrgChart.orientation.right_top:
                    O = OrgChart.ui._linkPpRight(p, t, C, m, g, l);
                    break;
                case OrgChart.orientation.left:
                case OrgChart.orientation.left_top:
                    O = OrgChart.ui._linkPpLeft(p, t, C, m, g, l)
            } else if ((C.isAssistant || 2 == C.layout) && C.rightNeighbor && C.rightNeighbor.isSplit) switch (o.orientation) {
                case OrgChart.orientation.top:
                case OrgChart.orientation.top_left:
                    O = OrgChart.ui._linkRightToLeft(C.rightNeighbor, C, l, h);
                    break;
                case OrgChart.orientation.bottom:
                case OrgChart.orientation.bottom_left:
                    O = OrgChart.ui._linkRightToLeft(C.rightNeighbor, C, l, h);
                ...
                case OrgChart.orientation.right:
                case OrgChart.orientation.right_top:
                    O = OrgChart.ui._linkBottomToTop(C.rightNeighbor, C, l, h);
                    break;
                case OrgChart.orientation.left:
                case OrgChart.orientation.left_top:
                    O = OrgChart.ui._linkBottomToTop(C.rightNeighbor, C, l, h)
            } else if ((C.isAssistant || 2 == C.layout) && C.leftNeighbor && C.leftNeighbor.isSplit) switch (o.orientation) {
                case OrgChart.orientation.top:
                case OrgChart.orientation.top_left:
                    O = OrgChart.ui._linkLeftToRight(C.leftNeighbor, C, l, h);
                    break;
                case OrgChart.orientation.bottom:
                case OrgChart.orientation.bottom_left:
                    O = OrgChart.ui._linkLeftToRight(C.leftNeighbor, C, l, h);
                ...
                case OrgChart.orientation.right:
                case OrgChart.orientation.right_top:
                    O = OrgChart.ui._linkTopToBottom(C.leftNeighbor, C, l, h);
                    break;
                case OrgChart.orientation.left:
                case OrgChart.orientation.left_top:
                    O = OrgChart.ui._linkTopToBottom(C.leftNeighbor, C, l, h)
                ...
                case OrgChart.orientation.top:
                case OrgChart.orientation.top_left:
                    if (1 == C.isPartner) O = OrgChart.ui._linkLeftToRight(t, C, l, d);
                    else if (2 == C.isPartner) O = OrgChart.ui._linkRightToLeft(t, C, l, d);
                    else {
                        var y = 1 == C.layout ? void 0 : m.minY - (m.minY - g.maxY) / 2;
                        O = OrgChart.ui._linkTopToBottom(t, C, l, h, y)
                    }
                    break;
                case OrgChart.orientation.bottom:
                case OrgChart.orientation.bottom_left:
                    if (1 == C.isPartner) O = OrgChart.ui._linkLeftToRight(t, C, l, d);
                        ...
                        y = 1 == C.layout ? void 0 : m.maxY - (m.maxY - g.minY) / 2;
                        O = OrgChart.ui._linkBottomToTop(t, C, l, h, y)
                    }
                    break;
                case OrgChart.orientation.right:
                case OrgChart.orientation.right_top:
                    if (1 == C.isPartner) O = OrgChart.ui._linkTopToBottom(t, C, l, d);
                    else if (2 == C.isPartner) O = OrgChart.ui._linkBottomToTop(t, C, l, d);
                    else {
                        y = 1 == C.layout ? void 0 : m.maxX - (m.maxX - g.minX) / 2;
                        O = OrgChart.ui._linkRightToLeft(t, C, l, h, y)
                    }
                    break;
                case OrgChart.orientation.left:
                case OrgChart.orientation.left_top:
                    if (1 == C.isPartner) O = OrgChart.ui._linkTopToBottom(t, C, l, d);
                        ...
                        y = 1 == C.layout ? void 0 : m.minX - (m.minX - g.maxX) / 2;
                        O = OrgChart.ui._linkLeftToRight(t, C, l, h, y)
                    }
            }
            if (-1 != x.indexOf("{rounded}"))
                if (O.xa == O.xb && O.xa == O.xc && O.xa == O.xd || O.ya == O.yb && O.ya == O.yc && O.ya == O.yd) x = x.replace("{rounded}", "M" + O.xa + "," + O.ya + " L" + O.xd + "," + O.yd);
                else {
                    var v = OrgChart.ui._roundedEdge(O.xa, O.ya, O.xb, O.yb, O.xc, O.yc),
                        b = OrgChart.ui._roundedEdge(O.xb, O.yb, O.xc, O.yc, O.xd, O.yd);
                    x = x.replace("{rounded}", "M" + v.x1 + "," + v.y1 + " " + v.x2 + "," + v.y2 + " Q" + v.qx1 + "," + v.qy1 + " " + v.qx2 + "," + v.qy2 + " L" + b.x2 + "," + b.y2 + " Q" + b.qx1 + "," + b.qy1 + " " + b.qx2 + "," + b.qy2 + " L" + b.x3 + "," + b.y3)
                }
            else x = -1 != x.indexOf("{edge}") ? x.replace("{edge}", "M" + O.xa + "," + O.ya + " " + O.xb + "," + O.yb + " " + O.xc + "," + O.yc + " L" + O.xd + "," + O.yd) : -1 != x.indexOf("{curve}") ? x.replace("{curve}", "M" + O.xa + "," + O.ya + " C" + O.xb + "," + O.yb + " " + O.xc + "," + O.yc + " " + O.xd + "," + O.yd) : x.replaceAll("{xa}", O.xa).replaceAll("{ya}", O.ya).replaceAll("{xb}", O.xb).replaceAll("{yb}", O.yb).replaceAll("{xc}", O.xc).replaceAll("{yc}", O.yc).replaceAll("{xd}", O.xd).replaceAll("{yd}", O.yd);
            s.push(OrgChart.linkOpenTag.replace("{id}", t.id).replace("{class}", "link " + C.tags.join(" ")).replace("{child-id}", C.id));
            var w = {
                node: t,
                cnode: C,
                p: O,
            ...
            "" != _ && (_ = OrgChart.linkFieldsOpenTag.replace("{x}", O.x).replace("{y}", O.y).replace("{rotate}", 0) + _ + OrgChart.grCloseTag, s.push(w.html)), s.push(OrgChart.grCloseTag)
        }
        return s.join("")
    },
    ...
    if ("multiline" == s) {
        ...
        C.setAttributeNS(null, "x", a), C.setAttributeNS(null, "y", n), C.setAttributeNS(null, "text-anchor", o), C.appendChild(m), d.appendChild(C);
        p = 1;
        for (var O = 1, x = 1; x < f.length; x++) {
            var y = C.firstChild.data.length;
            if (C.firstChild.data += " " + f[x], C.getComputedTextLength() > l) {
                if (C.firstChild.data = C.firstChild.data.slice(0, y), O++, 0 !== c && c < O) break;
                (C = document.createElementNS(h, "tspan")).setAttributeNS(null, "x", a), C.setAttributeNS(null, "y", n + u * p), C.setAttributeNS(null, "text-anchor", o), m = document.createTextNode(f[x]), C.appendChild(m), d.appendChild(C), p++
            }
        }
        var v = "";
    ...
    for (var r, i, a, n, o, l = "", s = 0; s < this.config.clinks.length; s++) {
        ...
        if (-1 != e.res.visibleNodeIds.indexOf(h.from) && -1 != e.res.visibleNodeIds.indexOf(h.to)) {
            ...
            switch (this.config.orientation) {
                case OrgChart.orientation.top:
                case OrgChart.orientation.top_left:
                    O <= x ? (b = 1, C.x = O + d.w / 10, m.x = x - c.w / 10) : (b = -1, C.x = O - d.w / 10, m.x = x + c.w / 10), p == u ? (b = 1, C.y = p, m.y = u) : u < p ? (C.y = p, m.y = u + c.h) : (C.y = p + d.h, m.y = u);
                    break;
                case OrgChart.orientation.bottom:
                case OrgChart.orientation.bottom_left:
                    C.x = O <= x ? (b = -1, O + d.w / 10) : (b = 1, O - d.w / 10), m.x = x + c.w / 10, p == u ? (b = -1, C.y = p + d.h, m.y = u + c.h) : u < p ? (C.y = p, m.y = u + c.h) : (C.y = p + d.h, m.y = u);
                    break;
                case OrgChart.orientation.left:
                case OrgChart.orientation.left_top:
                    C.y = y <= v ? (b = -1, y + d.h / 5) : (b = 1, y - d.h / 5), m.y = v + c.h / 5, g == f ? (b = -1, C.x = g, m.x = f) : f < g ? (C.x = g, m.x = f + c.w) : (C.x = g + d.w, m.x = f);
                    break;
                case OrgChart.orientation.right:
                case OrgChart.orientation.right_top:
                    C.y = y <= v ? (b = 1, y + d.h / 5) : (b = -1, y - d.h / 5), m.y = v + c.h / 5, g == f ? (b = 1, C.x = g + d.w, m.x = f + c.w) : f < g ? (C.x = g, m.x = f + c.w) : (C.x = g + d.w, m.x = f)
            }
            var w = I(C, m, b),
                _ = h.template;
            _ || (_ = "orange");
            var k = OrgChart.clinkTemplates[_];
            l += '<g c-link-from="{from}" c-link-to="{to}">'.replace("{from}", d.id).replace("{to}", c.id) + k.link.replaceAll("{d}", "M{a.x},{a.y} C{a.x},{a.y} {c.x},{c.y} {b.x},{b.y}").replaceAll("{b.x}", m.x).replaceAll("{b.y}", m.y).replaceAll("{a.x}", C.x).replaceAll("{a.y}", C.y).replaceAll("{c.x}", w.x).replaceAll("{c.y}", w.y);
            var S = (r = C, a = w, void 0, n = ((i = m).x - r.x) / 2 + r.x, o = (i.y - r.y) / 2 + r.y, {
                x: (n - a.x) / 2 + a.x,
                y: (o - a.y) / 2 + a.y
            });
    ...
    function t(t, e, r) {
            ...
            for (var c = t; null == a;) {
                ...
                if (!g) {
                    ...
                    if (p.parent) switch (h = l.levelSeparation, p.parent.layout != OrgChart.mixed && p.parent.layout != OrgChart.tree || (h = l.mixedHierarchyNodesSeparation), l.orientation) {
                        ...
                        case OrgChart.orientation.left:
                        case OrgChart.orientation.left_top:
                            C = p.parent.x + p.parent.w + h * (2 / 3), m = i[i.length - 1][1];
                            break;
                        case OrgChart.orientation.right:
                        case OrgChart.orientation.right_top:
                            C = p.parent.x - h * (2 / 3), m = i[i.length - 1][1]
                    }
                    i.push([C, m])
                }
                c = p
        ...
        O || (O = "orange");
        var x = null;
        switch ((s = OrgChart.slinkTemplates[O]).labelPosition) {
            case "start":
                x = {
                    x: i[1][0],
                    y: i[1][1]
        ...
        if (A += '<g c-link-from="{from}" c-link-to="{to}">'.replace("{from}", t.id).replace("{to}", e.id) + s.link.replaceAll("{d}", b), E.label) {
            var w = s.label.replace("{x}", x.x).replace("{y}", x.y).replace("{val}", E.label),
            ...
            A += s.label.replace("{x}", x.x).replace("{y}", x.y + k).replace("{val}", E.label)
        }
    ...
    if (t.config.miniMap) {
        ...
        var h = 0;
        drawnNodes = [], checkedLevel = [],
            function t(e, r) {
                if (Array.isArray(r))
                    for (var i = 0; i < r.length; i++) t(e, r[i]);
                else {
        ...
        var d = t.getViewBox()[0],
            ...
            g = t.getViewBox()[2],
            p = t.getViewBox()[3];
        a.lineWidth = .5 / o, a.strokeStyle = "#f57c00", a.strokeRect(d, c, g, p)
    }
}), OrgChart.miniMap = {}, OrgChart.miniMap._getCanvas = function (t) {
    if (!(e = t.element.querySelector('[data-id="mini-map"]'))) {
        var e;
        (e = document.createElement("canvas")).width = 250, e.height = 140, e.setAttribute("data-id", "mini-map"), e.style.display = "inline-block";
        var r = document.createElement("div");
        r.style.position = "absolute", r.style.bottom = "10px", r.style.left = "10px", r.style.border = "1px solid #aeaeae", r.style.padding = "5px", r.style.margin = t.config.padding + "px", r.appendChild(e), t.element.appendChild(r)
    }
    return e
}, OrgChart.miniMap.colors = ["#E0E0E0", "#BDBDBD", "#9E9E9E", "#757575"], OrgChart.events.on("redraw", function (t, e) {
    if (t.config.state) {
        var i = [],
            a = [];
        !function t(e) {
