...
class AccountInvoiceDispute(models.Model):
    _name = "account.invoice.dispute"
    _description = 'Account Invoice Dispute for dispute suppliers'
    _order = 'id desc'

    name = fields.Char(string='Dispute Reference', copy=False, readonly=True,
    ...
    date = fields.Date(string='Date', required=True, readonly=True, index=True,
                       states={'draft': [('readonly', False)],
                               'dispute': [('readonly', False)]}, copy=False,
    ...
    def create(self, vals):
        dispute_name = self.env['ir.sequence'].next_by_code(
            'sequence.supplier.dispute.code')
        vals.update({'name': dispute_name})

        res = super(AccountInvoiceDispute, self).create(vals)
        return res
