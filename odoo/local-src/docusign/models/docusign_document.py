...
class DocusignDocument(models.Model):
    ...
    def _default_docusign_server_id(self):
        ...
        '''
        docusign_server_id = self.env['docusign.config'].search([('state',
                                                                  '=',
                                                                  'confirm')],
                                                                limit=1
                                                         )
    ...
    partner_ids = fields.Many2many('res.partner',
        ...
        'document_id', 'partner_id', 'Additional contacts')
    attachment_ids = fields.Many2many('ir.attachment',
        'docusign_email_ir_attachments_rel',
        'document_id', 'attachment_id', 'Attachments')
    sign_attachment_ids = fields.Many2many('ir.attachment',
        'docusign_email_ir_sign_attachments_rel',
        'sign_document_id', 'sign_attachment_id', 'Attachments')
    ...
    def send_document_cron(self):
        ...
        if docusign_email_ids:
            for email_id in self:
                ...
                signature = email_id.docusign_template_id and \
                    email_id.docusign_template_id.signhere_tab
                xoff = email_id.docusign_template_id and \
                    email_id.docusign_template_id.xoff
                yoff = email_id.docusign_template_id and \
                    email_id.docusign_template_id.yoff
                datesigned = email_id.docusign_template_id and \
                    email_id.docusign_template_id.signhere_tab or False
                xoff_date = email_id.docusign_template_id and \
                    email_id.docusign_template_id.xoff_date or 0.00 
                yoff_date = email_id.docusign_template_id and \
                    email_id.docusign_template_id.yoff_date or 0.00
                _logger.info(("__________docusign__mail_id___________ %s") % \
                            (email_id))
        ...
        for email_id in self:
            ...
            if content_status.get('status') == 'completed':
                ...
                if response_doc.get('status') != '200':
                    ...
                    email_id.write({'sign_attachment_ids': [(6,
                                                             0,
                                                             attach_ids)]})
