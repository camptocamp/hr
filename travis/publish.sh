...
if [ "$TRAVIS_PULL_REQUEST" == "false" ]; then
  ...
  if [ "$TRAVIS_BRANCH" == "master" ]; then
    ...
    docker tag roctool_odoo camptocamp/roctool_odoo:latest
    docker push "camptocamp/roctool_odoo:latest"
    echo "Building test server"
    deploy latest
  elif [ ! -z "$TRAVIS_TAG" ]; then
    echo "Deploying image to docker hub for tag ${TRAVIS_TAG}"
    docker tag roctool_odoo camptocamp/roctool_odoo:${TRAVIS_TAG}
    docker push "camptocamp/roctool_odoo:${TRAVIS_TAG}"
  elif [ ! -z "$TRAVIS_BRANCH" ]; then
    echo "Deploying image to docker hub for branch ${TRAVIS_BRANCH}"
    docker tag roctool_odoo camptocamp/roctool_odoo:${TRAVIS_BRANCH}
    docker push "camptocamp/roctool_odoo:${TRAVIS_BRANCH}"
  else
